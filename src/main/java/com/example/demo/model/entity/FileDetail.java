...
public class FileDetail {
    ...
    private String tradeUniqueId;        // D: 交易唯一值 (例：202508120000001)

    @Column(name = "trade_time", nullable = false, length = 14)
    private String tradeTime;            // D: 交易時間 (yyyyMMddHHmmss)

    @Column(name = "merchant_no", length = 20)
    private String merchantNo;           // D: 商店代號 (例：P10012-1)

    @Column(name = "order_no", length = 20)
    private String orderNo;              // D: 訂單編號 (若有就填)

    @Column(name = "trade_amount", nullable = false)
    private Integer tradeAmount;         // D: 交易金額（INT）

    @Column(name = "discount_amount", nullable = false)
    private Integer discountAmount;      // D: 折扣金額（INT）

    @Column(name = "pay_amount", nullable = false)
    private Integer payAmount;           // D: 請款金額（INT）
}
